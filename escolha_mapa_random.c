...
int randomizarMapa(Texture2D *fundoAtual, Texture2D fundos[], int indiceAtual){

    int novoIndice;

    // Gera um novo índice aleatório
    do {
        novoIndice = rand() % MAX_BACKGROUNDS;
    } 
    while (novoIndice == indiceAtual);

    *fundoAtual = fundos[novoIndice];
...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        {
            indiceAtual = randomizarMapa(&fundoAtual, fundos, indiceAtual);
        }
